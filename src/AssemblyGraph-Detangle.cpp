...
void AssemblyGraph::splitSelfComplementaryTangle(const Tangle& tangle) const
{
    // EXPOSE WHEN CODE STABILIZES.
    ...
    if(debug) {
        cout << "AssemblyGraph::splitSelfComplementaryTangle begins." << endl;
        cout << "This tangle has " << tangle.tangleEdges.size() << " segments." << endl;
    }
    SHASTA2_ASSERT(tangle.isSelfComplementary());
    ...
    if(debug) {
        ...
        for(uint64_t segmentPairIndex=0; segmentPairIndex<segmentPairs.size(); segmentPairIndex++) {
            ...
            cout << segmentPairIndex << ": " << id(segment) << " " << id(segmentRc) << endl;
        }
    }
    ...
    if(debug) {
        cout << "Segments:" << endl;
        for(uint64_t segmentIndex=0; segmentIndex<segments.size(); segmentIndex++) {
            const Segment segment = segments[segmentIndex];
            cout << segmentIndex << ": " << id(segment) << endl;
        }
    }
    ...
    if(debug) {
        ofstream csv("splitSelfComplementaryTangle.csv");
    ...
    if(debug) {
        cout << "AssemblyGraph::splitSelfComplementaryTangle ends." << endl;
    }
}
